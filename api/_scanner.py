...
def get_scan_results(
    ...
    try:
        ...
        # Fallback if hudson is not supported in this version
        if "hudson" in str(e).lower():
            print("Warning: Hudson Rock not supported in current user-scanner version")
        raise

    # Apply only_found filter
    if only_found:
        results = [r for r in results if getattr(r, 'is_found', lambda: False)()]
